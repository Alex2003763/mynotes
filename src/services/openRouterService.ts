...
export const summarizeText = async (text: string, length: 'short' | 'bullet', language: 'en' | 'zh' = 'en', modelId: string): Promise<string> => {
  const langInstruction = language === 'zh' ? '请用中文回答。' : '';
  const prompt = length === 'short' 
    ? `Summarize the following text into a concise paragraph. ${langInstruction}\n\nText:\n${text}`
    : `Extract the key points from the following text and present them as a bulleted list. ${langInstruction}\n\nText:\n${text}`;
  
  const messages: OpenRouterMessage[] = [
    { role: 'system', content: `You are an expert summarizer. Provide clear and concise summaries. ${langInstruction}` },
...
export const correctGrammarAndSpelling = async (text: string, language: 'en' | 'zh' = 'en', modelId: string): Promise<string> => {
  const langInstruction = language === 'zh' 
    ? '你是一个中文语法和拼写校对专家。请修正文本中的语法和拼写错误。仅返回修正后的文本。如果不需要修正，请返回原文。' 
    : 'You are an expert proofreader. Correct grammar and spelling mistakes. Return only the corrected text. If no corrections are needed, return the original text.';
...
export const expandContent = async (selectedText: string, instruction: string, language: 'en' | 'zh' = 'en', modelId: string): Promise<string> => {
  const langSystemInstruction = language === 'zh'
    ? '你是一位富有创造力的写作助手。根据用户的指示帮助扩展和阐述给定的文本。'
